...
class Communication:
    ...
    signature = None
    hashofthemessage = None
    hashofthekey = None
    hashoftherecievedkey = None
    hashoftherecievedmessage = None
    ...
    def sendMessage(self, reciecerpublickey):
        ...
        digest.update(bytes(str(__symmentrickey), 'utf-8'))
        self.hashofthekey = digest.finalize()
        digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
        digest.update(bytes(str(__message), 'utf-8'))
        self.hashofthemessage = digest.finalize()
    ...
    def decryptRecievedMsg(self, recieversymmentrickey, reciveriv, ct, keyhash, messagehash):
        ...
        digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
        digest.update(bytes(str(recieversymmentrickey), 'utf-8'))
        self.hashoftherecievedkey = digest.finalize()
        digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
        digest.update(bytes(str(decryptedmessage.decode("ascii")), 'utf-8'))
        self.hashoftherecievedmessage = digest.finalize()

        if self.hashoftherecievedkey == keyhash and self.hashoftherecievedmessage == messagehash:
...
while True:
    ...
    if n == 4:
        ...
        bob.decryptRecievedMsg(bob.recieversymmentrickey, bob.recieveriv, alice.ciphermessage,alice.hashofthekey,alice.hashofthemessage)
    if n == 5:
        print("Sending message to alice...")
        bob.sendMessage(alice.usablepublickey)
        print("Message sent successfully.")
    if n == 6:
        print("Receiving message from Bob... ")
        alice.recieveMessage(bob.usablepublickey, bob.symmentrickeyCipher, bob.ivCipher, bob.signature)
        print("Decrypting and Printing the message...")
        alice.decryptRecievedMsg(alice.recieversymmentrickey, alice.recieveriv, bob.ciphermessage,bob.hashofthekey,bob.hashofthemessage)
    if n == 7:
        break
